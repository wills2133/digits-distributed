...
class Task(StatusCls):
    ...
    def run_bkp(self, resources):
        ...
        # args[2] = '--solver=/home/wills/Projects/caffe-ssd/models/VGGNet/VOC0712/SSD/solver.prototxt'
        ...
        # change solvertxt by wills
        # solvertxt = args[2].split('=', 1)[1]
        # direct = os.path.split(solvertxt)[0]
        # shutil.copyfile("/home/wills/Projects/digits/digits/jobs/solver_voc.prototxt",solvertxt)
        # raw_input("pause_task")
    ...
    def run(self, resources):
        ...
        try:
            ...
            thread_log = client.training_request( (' ').join(args) , self.job_dir, self.job_id)
            n = 0
            while ( not thread_log.stopped ) or ( n < len( thread_log.log_list ) ):
                while n < len( thread_log.log_list ):
                    ...
                    if self.aborted.is_set():
                        if sigterm_time is None:
                            client.abort_request(self.job_dir, self.job_id)
                            thread_log.stop()
                            # Attempt graceful shutdown
                            # self.p.send_signal(signal.SIGTERM)
                            sigterm_time = time.time()
