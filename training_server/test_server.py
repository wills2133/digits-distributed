...
class thread_test_job(threading.Thread):
    def __init__(self, proto_req, proto_res, tcp):
        threading.Thread.__init__(self)
        self.tcp = tcp
        self.interval = interval
        self.proto_req = proto_req
    ...
    def run(self):
        ...
        self.proto_res.test_args = 'caffe/predict jobxxxx/model'
        self.tcp.send_message(self.proto_res) # send test arguments
        ...
        try:
            ...
            while (model_seg) and (not self.stopped)::
                ...
                self.proto_res.model_seg = model_seg
                self.proto_res.test_args = '--transfering'
                self.tcp.send_message(self.proto_res)

            self.proto_res.test_args = '--transfered'
            self.tcp.send_message(self.proto_res)
            f.close()
...
def run_training_server(ip, port):
    ...
    while True:
        try:
            ...
            if request.command == proto.Request.SEND:
                ...
                test_job = thread_test_job(request, tcp_test)
                test_job.start()
